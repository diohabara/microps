...
static int ether_tap_addr(struct net_device *dev) {
  ...
  ifr.ifr_addr.sa_samily = AF_INET;
  ...
  memcpy(dev->adr, ifr.ifr_hwaddr.sa_data, ETHER_ADDR_LEN);
  close(soc);
  return 0;
}
...
static int ether_tap_open(struct net_device *dev) {
  ...
  tap = PRIV(dev);
  tap->fd = open(CLONE_DEVICE, O_PDWR);
  if (tap->fd == -1) {
...
static int ether_tap_poll(struct net_device *dev) {
  ...
  case -1:
    if (errno != EINTR) {
      errof("read: %s, dev=%s", strerror(errno), dev->name);
    }
    /* fall through */
  case 0:
    return -1;
