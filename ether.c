...
int ether_transmit_helper(struct net_device *dev, uint16_t type,
                          ...
                          ssize_t (*callback)(struct net_device *dev,
                                              const uint8_t *data,
                                              size_t len)) {
  uint8_t fram[ETHER_FRAME_SIZE_MAX] = {};
  struct ether_hdr *hdr;
  size_t flen, pad = 0;

  hdr(struct ether_hdr *) fram;
...
int ether_poll_helper(struct net_device *dev,
                      ssize_t (*callback)(struct net_device *dev, uint8_t *buf,
                                          size_t size)) {
...
void ether_setup_helper(struct net_device *dev) {
  ...
  memcpy(dev->broadcast, ETHER_ADR_BROADCAST, ETHER_ADDR_LEN);
}
