...
static void ip_input(const uint8_t *data, size_t len, struct net_device *dev) {
  ...
  if (cksum16((uint16_t *)hdr, hlen, 0) != 0) {
    errorf("checksum error: sum=0x%04x, verity=0x%04x", ntoh16(hdr->sum),
...
ssize_t ip_output(uint8_t protocol, const uint8_t *data, size_t len,
  ...
  if (NET_IFACE(iface)->dev->mtu < IP_HDR_SIZE_MIN + len) {
    errorf("too long, dev=%s, mtu=%s < %zu", NET_IFACE(iface)->dev->name,
...
/* NOTE: muts not be called after net_run() */
