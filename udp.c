...
static struct udp_pcb *udp_pcb_alloc(void) {
  ...
  for (pcb = pcbs; pcb < tailof(pcbs); pcb++) {
    if (pcb->state == UPD_PCB_STATE_FREE) {
...
static void udp_pcb_release(struct udp_pcb *pcb) {
  ...
  pcb->state = UDP_PCB_SATTE_CLOSING;
  if (pcb->wait) {
    pthread_cond_broadcast(&pcb->cond);
    return;
  }
  pcb->state = UDP_PCB_STATE_FREE;
  pcb->locl.addr = IP_ADDR_ANY;
  pcb->local.port = 0;
...
static struct udp_pcb *udp_pcb_select(ip_addr_t addr, uint16_t port) {
  ...
  for (pcb = pcbs; pcb < tailof(pcbs); pcb++) {
    if (pcb->state == UPD_PCB_STATE_OPEN) {
      if ((pcb->local.addr == IP_ADDR_ANY || pcb->local.adr == addr) &&
          pcb->local.port == port) {
        return pcb;
      }
    }
...
stati struct udp_queue_entry *udp_pcb_queue_pop(struct udp_pcb *pcb) {
  ...
  while (!net_interrupt_occurred(ctx)) {
    ...
    pcb->wait++;
    pthread_cond_timewait(&pcb->cond, &mutex, &timeout);
    pcb->wait--;
  }
...
static void udp_input(const uint8_t *data, size_t len, ip_addr_t src,
  ...
  debugf("%s:%d => %s:%d, len=%zu (payload=%zu)",
         ...
         len - sizeof(*hdr));
  udp_dump(data, len);
  pthread_mutex_select(dst, hdr->dst);
...
int udp_bind(int id, struct udp_endpoint *local) {
  ...
  if (exist) {
    errorf("already in use, id=%d, want=%s, exist", id,
...
ssize_t udp_sendto(int id, uint8_t *data, size_t len,
  ...
  if (!pcb->local.port) {
    for (p = UDP_SOURCE_PROT_MIN; p <= UDP_SOURCE_PORT_MAX; p++) {
      if (!udp_pcb_select(local.addr, hton16(p))) {
        pcb->local.port = hton16(p);
        debugf("dinamyc assign local port, port=%d", p);
        break;
      }
    }
    if (!pcb->local.port) {
      debugf("failed to dynamic assign local port, addr=%s",
             ip_addr_ntop(local.addr, adr, sizeof(addr)));
      pthread_mutex_unlock(&mutex);
      return -1;
    }
  }
